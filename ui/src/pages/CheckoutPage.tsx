...
const CheckoutPage = (() => {
    ...
    useEffect(() => {
        ...
        postData({ url: "/api/user/current/", data: form_data, setActiveSpinner: setSpinnerActive }).then(response => {
    ...
    useEffect(() => {
        let sum = 0;
        let shippingSum = 0;
        products.forEach((object: Product) => {sum += object.price; shippingSum += object.shipping_price});
        setCartSum(sum);
    ...
    const SubmitForm = (e: React.FormEvent<HTMLFormElement>) => {
        ...
        if (checkoutForm.current) {
            ...
            const token = cookies.get("token");
            form_data.append("token", token);
            products.forEach((object: Product) => { form_data.append("items[]", String(object.id)); })
            let id = 0;
            postData({ url: "/api/order/create/", data: form_data, setActiveSpinner: setSpinnerActive }).then(response => {
                if (response && response.ok) {
                    return response.json();
                }
                throw response;
            ...
            <form ref={checkoutForm} onSubmit={SubmitForm} className="checkout_form">
                ...
                <div className="checkout_summary">
                    <div className="checkout_products_container">
                        {products.map((object: Product) => {return <ProductComponent product={object} size={0} />})}
                    </div>
                    <div className="checkout_prices_button">
                        <div className="checkout_prices">
                            <div><span>Product price:</span>{cartSum} zł</div>
                            <div><span>Shipping price:</span> {cartShipping} zł</div>
                            <div id="total_price"><span>Total price:</span> {cartSum + cartShipping} zł</div>
                        </div>
